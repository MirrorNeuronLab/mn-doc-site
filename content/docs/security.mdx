...
description: Overview of security considerations, trust boundaries, and safe practices for the MirrorNeuron runtime.
---

# Security Model

Use this page before you run a third-party blueprint, handle sensitive data, expose a listener, or join a runtime node to a cluster. MirrorNeuron can execute code, read declared files, use declared environment variables, and call configured services; the operator is responsible for selecting and protecting those boundaries.

## Before you begin

- Know which runner each worker uses: HostLocal or OpenShell.
- Review the blueprint `manifest.json`, payloads, and `pass_env` entries.
- Keep Redis, API, gRPC, and artifact-sharing interfaces on a trusted network.
- Do not use real secrets or regulated data for an unreviewed blueprint.
...
- **Redis to runtime:** Redis is the lease and support authority; protect it like a control-plane database.
...
- Use Open1Skill for code you do not fully trust.
...
- [Blueprints and Skills](blueprints-and-skills.mdx)
- [Redis High Availability](redis-ha.mdx)
