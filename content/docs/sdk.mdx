...
description: Documentation for the MirrorNeuron Python SDK, covering installation, usage, and core concepts.
---
...
After compiling a decorated workflow into a bundle, validate the generated folder:

```bash
mn blueprint validate /path/to/generated-bundle
```
...
| `backpressure` | Node-length queue and pressure policy. |
...
Keep `pass_env` narrow. See [Security Model](security.mdx).
...
| `policies` | Retry, reschedule, scheduler, and update policy maps. |
...
See [Nomad-Inspired Runtime Features](nomad-inspired-runtime.mdx), [Services and Health Checks](services-and-health-checks.mdx), [Resources and Devices](resources-and-devices.mdx), [Deployments](deployments.mdx), and [Schedules and Events](schedules-and-events.mdx).
...
For a source-mode blueprint, keep the generated bundle out of the repository,
validate it locally, and include only the source workflow and declared payloads
in version control.

## Related Pages

- [Blueprints and Skills](blueprints-and-skills.mdx)
- [Job Bundle Format](bundle.mdx)
