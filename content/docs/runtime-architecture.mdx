...
description: Explains the runtime model behind MirrorNeuron and its design logic.
---
...
- Pools and slot accounting for heavier execution capacity
- Strong message and artifact boundaries between scheduler concerns and task payload concerns

Airflow's big lesson for this runtime is not "copy operators." It is "treat heavier execution capacity as scarce and schedule it explicitly."
...
The current DAG execution model is a static DAG scheduler with per-step lifecycle state. It is not a full GOAP or PDDL planner yet.
...
- supervision
- persistence
- placement scheduling and execution metadata
- retries and backoff
- event history
- aggregation
...
## Logical worker vs physical execution lease
...
Before this change, each executor agent launched Open1 directly from its own process. That worked for small demos, but it broke down under large fan-out:
...
- Every executor requests a lease before running Open1.
...
- `artifacts`: references to larger externally-offloaded data
- `stream`: optional stream framing metadata

This keeps the runtime generic. The runtime should understand enough to route and observe messages, but it should not need to parse every application payload to work correctly.
...
The detailed map is in [Nomad-Inspired Runtime Features](nomard-inspired_runtime.md).

## Why artifacts matter

Passing large blobs directly between agents is a bad fit for a clustered runtime. Instead, messages should stay small and carry:
...
This is the same core operational lesson many schedulers learn: small control messages scale much better than inline larger payloads.
...
The project is not BEAM-only in the sense of "all useful code must be Elixir." That would make the runtime less practical. Instead:

- BEAM owns coordination
- OpenShell owns isolated execution
- worker payloads can be shell, Python, or other supported runs
...
---
unlink(content/docs/md-legacy/runtime-architecture.md)
