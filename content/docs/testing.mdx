...
- Create the workspace virtual environment described in [Contributing](/contributing), then use `.venv/bin/python` so the command does not depend on shell activation.
