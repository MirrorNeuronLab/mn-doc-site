---
title: Testing Guide
description: A guide to testing strategies, layouts, and common commands for the MirrorNeuron project.
---
...
- Run commands from the workspace root unless a command says otherwise.
- Create the workspace virtual environment described in [Contributing](contributing.md), then use `.venv/bin/python` so the command does not depend on shell activation.
- Start Docker, Redis, or local services only for a test tier that requires them.
...
- `tests/e2e/`: runtime workflows, live API/CLI flows, Docker or service-backend checks.
...
- `otterdesk-blueprints/*/tests` and `mn-skills/*/tests`: blueprint and skill package tests.
...
For a cross-component regression, prefer an injected contract test before adding a live test. Add live integration or end-to-end coverage only when the behavior depends on a running service, Docker, Redis, network topology, or another external capability that an injected test cannot represent.
