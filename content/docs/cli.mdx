...
This page is a task-oriented reference. The installed CLI help is authoritative for the exact version on your machine:

```bash
mn --help
mn <command-group> --help
```

Replace `<command-group>` with `blueprint`, `job`, `runtime`, `model`, `node`, `service`, `schedule`, or another group listed by `mn --help`.
...
Cluster helper scripts in `MirrorNeuron/scripts/` provide lower-level node start, join, Redis HA, and failover smoke-test flows. See [Cluster Guide](/cluster) and [Redis High Availability](/redis-ha).
...
mn node maintenance mirror_neuron@<node-host> --disable --reason "ready"
```
...
```bash
mn node drain mirror_neuron@<node-host> --reason "reboot" --deadline 30m --dry-run
...
See [Resources and Devices](/resources-and-devices).
...
See [Services and Health Checks](/services-and-health-checks).
...
See [Model Runtime](/model-runtime).
...
See [Schedules and Events](/schedules-and-events).
...
For controlled two-box startup, prefer the scripts in [Cluster Guide](/cluster).
