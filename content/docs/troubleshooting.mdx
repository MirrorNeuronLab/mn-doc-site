...
description: A collection of common operational failures and their fixes during local and two-box testing.
---
...
For a failed workflow, also record the job ID, run ID, timestamp, and exact error text. Use `mn job status <job_id>` and `mn job monitor <job_id>` to distinguish a runtime failure from a blueprint requirement or worker failure.
...
If the `mirror-neuron-redis` container is listed, check it directly:

```bash
docker exec mirror-neuron-redis redis-cli ping
```

`PONG` confirms that the container is responding. If the container is not running, start the managed runtime first:
...
Only use a manually started Redis container when you intentionally manage Redis outside the MirrorNeuron deployment. Warning: removing a Redis container can destroy the runtime state stored in that container. Back up or preserve the data before any destructive cleanup.
...
- the API sidecar fails to start

Fix:

- By default, the FastAPI gateway binds to port `54001`.
- Stop the conflicting process or choose another API port, for example: `export MN_API_PORT=54002`.
- Keep the API port distinct from the gRPC port (`55051` by default) and the Docker Model Runner/LiteLLM gateway port (`4000` by default).
